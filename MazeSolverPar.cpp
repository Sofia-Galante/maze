...
#pragma omp parallel default(none) shared(particles) \
firstprivate(numberOfParticles, seeds, maze) num_threads(threads)
    {
...
firstprivate(moves, maze) shared(rightMoves, lock) num_threads(4)
    for(int i = 0; i < 4; i++){
